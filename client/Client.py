...
class Mensagem():
    def __init__(self, fileName, Status):
        ...
        self.payloads = construirPayloads(mensagem=self.mensagem, Status=self.Status, size=113)
...
class Client():
    ...
    def packageEncoder(self):
        firstPkgPayload = FILE.encode()
        firstPkgHead = self.generateHeader(3,1,len(firstPkgPayload))
        firstPkg = firstPkgHead + firstPkgPayload + self.EOP
        self.packages = [firstPkg]
        for i in range(len(self.payloads)):
            header = self.generateHeader(3,i+2,len(self.payloads[i]),header7=i+1)
            self.packages.append(header+self.payloads[i]+self.EOP)
        self.Status.status('pkgs_encoded')
    ...
    def handshaking(self):
        ...
        while HANDSHAKING:
            ...
            handshake_response, nRx = self.com.getData(14, start=start_handshake)

            if handshake_response[0] == 2:
                self.Status.log('receb',handshake_response[0],len(handshake_response))
                HANDSHAKING = False
    ...
    def sendPackages(self):
        i = 0
        if not VERBOSE:
            self.pbar = tqdm(len(self.packages),unit='packages',desc='Pacotes Enviados:')
        while i < len(self.packages):
            ...
            while SENDING:
                ...
                self.Status.log('envio',3,len(self.packages[i]),i+1,len(self.packages))
