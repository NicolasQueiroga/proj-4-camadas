...
class Server:
    ...
    def read_head(self):
        try:
            head = self.com1.getData(self.head_size, self.timer2, hs=self.FLAGS['HS'])[0]
            if head == 'R':
    ...
    def read_payload(self):
        try:
            if self.msg_type == b'\x01':
                ...
                self.file_type = self.com1.getData(self.pl_size, self.timer2)[0].decode('ascii')
                self.FLAGS['PKG_ID_1'] = False
                self.status('got_filetype')

            elif self.msg_type == b'\x03':
                self.payload += self.com1.getData(self.pl_size, self.timer2)[0]
                self.status('got_pl')
    ...
    def check_eop(self):
        try:
            eop = self.com1.getData(self.eop_size, self.timer2)[0]
            if eop == self.EOP:
    ...
    def server_response(self):
        try:
            if self.FLAGS['PKG_ERROR']:
                self.com1.rx.clearBuffer()
                head = [self.msg_type_dict['error'], self.sensor_id, self.server_id, int(self.pkgs_qty).to_bytes(1, 'big'), b'\x00', self.msg_id, int(self.last_pkg_id+1).to_bytes(1, 'big'), int(self.last_pkg_id).to_bytes(1, 'big'), b'\x00', b'\x00', self.EOP]
                ...
                self.com1.rx.clearBuffer()
                head = [self.msg_type_dict['timeout'], self.sensor_id, self.server_id, int(self.pkgs_qty).to_bytes(1, 'big'), b'\x00', self.msg_id, int(self.last_pkg_id+1).to_bytes(1, 'big'), int(self.last_pkg_id).to_bytes(1, 'big'), b'\x00', b'\x00', self.EOP]
                send_head = b''.join(head)
                self.com1.sendData(send_head)
                self.FLAGS['TIMEOUT'] = False
                ...
                self.com1.rx.clearBuffer()
                head = [self.msg_type_dict['data'], self.sensor_id, self.server_id, int(self.pkgs_qty).to_bytes(1, 'big'), b'\x00', self.msg_id, int(self.last_pkg_id+1).to_bytes(1, 'big'), int(self.last_pkg_id).to_bytes(1, 'big'), b'\x00', b'\x00', self.EOP]
                send_head = b''.join(head)
                self.com1.sendData(send_head)
                self.FLAGS['TIMEOUT'] = False
                ...
                if self.msg_type == b'\x01':
                    head_response_list = [self.msg_type_dict['handshake-response'], self.sensor_id, self.server_id, int(self.pkgs_qty).to_bytes(1, 'big'), b'\x00', self.msg_id, b'\x00', int(self.last_pkg_id).to_bytes(1, 'big'), b'\x00', b'\x00', self.EOP]
                    head_response = b''.join(head_response_list)
                    self.com1.rx.clearBuffer()
                    self.com1.sendData(head_response)
                    self.status('sent_hs')
                    self.write_log('envio', msg_type=1)

                elif self.msg_type == b'\x03':
                    self.status('got_data')
                    dataOK_response_list = [self.msg_type_dict['data-ok'], self.sensor_id, self.server_id, int(self.pkgs_qty).to_bytes(1, 'big'), b'\x00', self.msg_id, b'\x00', int(self.last_pkg_id).to_bytes(1, 'big'), b'\x00', b'\x00', self.EOP]
                    ...
                    self.write_log('envio', msg_type=3)
    ...
    def write_log(self, type, msg_type=0, CRC=b'\x00\x00'):
        now = datetime.now(tz=timezone.utc)
        print(self.msg_type)
        if type == 'receb':
            log = f'{now} / {type} / {msg_type} / {self.pl_size} / {self.pkg_id} / {CRC}\n'
        elif type == 'envio':
            log = f'{now} / {type} / {msg_type} / {self.pl_size}\n'
